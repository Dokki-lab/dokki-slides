...
2. Upload `exports/<slug>.pptx` as a Dokki File with metadata containing `kind=dokki-slides-export`, `protocol=dokki-slides@1`, and the same revision. Pass the complete non-empty metadata object in the upload call; do not send `{}` as a placeholder.
