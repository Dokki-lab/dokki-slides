...
test("keeps package, Dokki, and Claude marketplace versions aligned", async () => {
  ...
  const dokkiManifest = JSON.parse(await fs.readFile(path.join(root, "skill", "dokki.skill.json"), "utf8"))
  ...
  const skill = await fs.readFile(path.join(root, "skill", "SKILL.md"), "utf8")
